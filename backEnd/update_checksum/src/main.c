...
#define OG_PATH "/home/julian/epitech/perso/pkmnEvent/tools/update_checksum/og.pgt"
...
int main(int ac, char **av)
{
    ...
    switch (ac) {
        case (3) :
            ...
            break;
        default:
            og_file = (unsigned char *) my_open_file(OG_PATH, &len);
            if ( !og_file )
                return 84;

            new_file = (unsigned char *) my_open_file(av[1], &len);
            if ( !new_file )
                return 84;
            file_name = av[1];
            break;
    ...
    write_new_checksum( file_name, new_file, len, new_checksum );
